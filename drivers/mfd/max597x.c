...
/*
 * Device driver for MAX5970 and MAX5978 IC
 *
...
static int max597x_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	max597x = devm_kzalloc(&i2c->dev, sizeof(*max597x),
		GFP_KERNEL);
	...
	max597x->regmap = devm_regmap_init_i2c(i2c, &max597x_regmap_config);
	if (IS_ERR(max597x->regmap)) {
		dev_err(&i2c->dev, "Failed to initialise regmap\n");
		return -EINVAL;
	}

	/* Save below data for use with mfd cells */
	max597x->irq = i2c->irq;
	max597x->dev = &i2c->dev;
