...
/* Drive mode registers automatically change state on writing '1'.
 * Only one drive mode can be active, which implies that
 * OpenDrain + internal pull-up isn't supported by this chip.
 *
 * Drive mode is independent from the direction register!
 */
	...
	switch(chip->gpio_chip.ngpio)
	{
		case 0 ... 20:
			for (i = 0; i < sizeof(cy8c9520a_bank_offs) - 1; i ++)
				if (! (gpio / cy8c9520a_bank_offs[i + 1]))
					break;
			break;
		case 21 ... 40:
		...
		case 41 ... 60:
			for (i = 0; i < sizeof(cy8c9560a_bank_offs) - 1; i ++)
				if (! (gpio / cy8c9560a_bank_offs[i + 1]))
					break;
	}
	return i;
}

static int cy8c95x0_bit_by_port(struct cy8c95x0_chip *chip, unsigned off)
{
	u8 bit_offset = 0;
	...
	{
	case 0 ... 20:
		bit_offset = off - cy8c9520a_bank_offs[i];
		break;
	case 21 ... 40:
		bit_offset = off - cy8c9540a_bank_offs[i];
		break;
	case 41 ... 60:
		bit_offset = off - cy8c9560a_bank_offs[i];
		break;
	}
	return 1 << bit_offset;
}
