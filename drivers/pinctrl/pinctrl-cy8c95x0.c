...
struct cy8c95x0_chip {
	...
	/* protect serialized access to the muxed registers */
	...
	/* Bitmap for Gport2 fixup */
	DECLARE_BITMAP(shiftmask, MAX_LINE);
	struct irq_chip irq_chip;

	int nport;	/* Number of gports in this chip */
...
static inline u8 cypress_get_port(struct cy8c95x0_chip *chip, unsigned int pin)
{
	/* Account for GPORT2 which only has 4 bits */
	return CY8C95X0_PIN_TO_OFFSET(pin) / BANK_SZ;
}

static int cypress_get_pin_mask(struct cy8c95x0_chip *chip, unsigned int pin)
{
	/* Account for GPORT2 which only has 4 bits */
...
static int cy8c95x0_write_regs_mask(struct cy8c95x0_chip *chip, int reg,
...
static int cy8c95x0_read_regs_mask(struct cy8c95x0_chip *chip, int reg,
				   unsigned long *val, unsigned long *mask)
{
	DECLARE_BITMAP(tmask, MAX_LINE);
	DECLARE_BITMAP(tval, MAX_LINE);
...
static int cy8c95x0_setup_gpiochip(struct cy8c95x0_chip *chip, int ngpio)
{
...
static int cy8c95x0_irq_setup(struct cy8c95x0_chip *chip, int irq)
{
...
static int cy8c95x0_setup_pinctrl(struct cy8c95x0_chip *chip)
{
...
static int device_cy8c95x0_init(struct cy8c95x0_chip *chip)
{
	DECLARE_BITMAP(ones, MAX_LINE);
...
static int cy8c95x0_probe(struct i2c_client *client)
{
	...
	chip->nport = DIV_ROUND_UP(CY8C95X0_PIN_TO_OFFSET(ngpios), BANK_SZ);

	switch (chip->driver_data & CY8C95X0_GPIO_MASK) {
	case 20:
	...
	ret = cy8c95x0_setup_gpiochip(chip, ngpios);
	if (ret)
		goto err_exit;
