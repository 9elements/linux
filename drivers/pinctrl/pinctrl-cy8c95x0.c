...
#define CY8C95X0_MUX_REGMAP_TO_OFFSET(x, p) (CY8C95X0_VIRTUAL + (x) - CY8C95X0_INTMASK + (p) * MUXED_STRIDE)

/**
 * struct cy8c95x0_pinctrl - driver data
 * @regmap:         Device's regmap. Only direct access registers.
 * @irq_lock:       IRQ bus lock
 * @i2c_lock:       Mutex to hold while updating the regmap cache
...
static const struct regmap_range_cfg cy8c95x0_ranges[] = {
	{
		...
		.range_max = CY8C95X0_VIRTUAL + MAX_BANK * MUXED_STRIDE,
...
static bool cy8c95x0_readable_register(struct device *dev, unsigned int reg)
{
	...
	for (i = 0; i < ARRAY_SIZE(cy8c95x0_ranges); i++) {
		if (reg >= cy8c95x0_ranges[i].range_min &&
		    reg <= cy8c95x0_ranges[i].range_max) {
			return true;
		}
	}
...
static const struct regmap_config cy8c95x0_i2c_regmap = {
	...
	.ranges = cy8c95x0_ranges,
	.num_ranges = ARRAY_SIZE(cy8c95x0_ranges),
	...
	.max_register = CY8C95X0_VIRTUAL + MAX_BANK * MUXED_STRIDE,
	.num_reg_defaults_raw = CY8C95X0_VIRTUAL + MAX_BANK * MUXED_STRIDE,
				    ...
				    unsigned long config)
{
	...
	case PIN_CONFIG_DRIVE_PUSH_PULL:
	case PIN_CONFIG_BIAS_DISABLE:
		ret = cy8c95x0_regmap_read(chip, CY8C95X0_DIRECTION, port, &dir);
		if (ret)
			goto out;
				     ...
				     unsigned int pin, bool input)
{
	...
	/* Only update drive mode in push-pull configurations */
	if (test_bit(pin, chip->push_pull))
		ret = cy8c95x0_regmap_write_bits(chip, input ? CY8C95X0_DRV_HIZ : CY8C95X0_DRV_PP_FAST,
						 port, bit, bit);
