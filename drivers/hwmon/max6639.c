			       ...
			       struct max6639_data *data)
{
	...
	dev_err(&client->dev, "Setting output mask to 0x%02X", val);
...

{
	...
	data->fullspeed_on_therm[i] = of_property_read_bool(child,
		"maxim,full-speed-on-therm");

	data->fan_fail_indication[i] = of_property_read_bool(np, "maxim,fanfail_indication");
...
static int max6639_probe_from_dt(struct i2c_client *client, struct max6639_data *data)
{
	...
	data->ot_indication[0] = !of_property_read_bool(np, "maxim,ot0_indication");
	data->ot_indication[1] = !of_property_read_bool(np, "maxim,ot1_indication");
	data->therm_indication[0] = !of_property_read_bool(np, "maxim,therm0_indication");
	data->therm_indication[1] = !of_property_read_bool(np, "maxim,therm1_indication");
...
static int max6639_probe(struct i2c_client *client)
{
	...
	for (i = 0; i < 2; i++) {
		...
		/* PWM 120/120 (i.e. 100%) */
		data->pwm[i] = 120;
		data->fullspeed_on_therm[i] = false;
		data->spin_up_enable[i] = false;
	}
